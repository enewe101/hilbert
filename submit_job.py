...
def add_arguments(parser):
    ...
    parser.add_argument('-slr', default=1e-3, type=float, nargs='+', dest='start_learning_rates',
                        help='give a series of start learning rates.')
    parser.add_argument('-u', default='100k', type=str, dest='num_updates',
                        help="give a number of updates, you can type (?)k or just a number")
    parser.add_argument('-p', default='10k', type=str, dest='batches',
                        help='number of batches, you can type (?)k or just a number')
    parser.add_argument('-o', default='/home/jingyihe/scratch/gibbs', type=str,
...
def write_python_command(**args):
    ...
    run_cmd = "python {} -b {} -o {} -l {} --writes=50 --updates={} --batch-size={}" \
              " --temperature={} -d 300 -thres 10 --gibbs --gibbs_iteration=1 -ga {}" \
              .format(
                    SAMPLE_MLE_PY,
                    args['cooc_dir'],
                    args['output_dir'],
...
def main(submit_dir, args):
    ...
    for lr in start_lrs:
        ...
        outfile_name = "_".join([ga_str, lr_str, batch_str, update_str, temp_str])
        # outfile_name += "_3steps"
        output_dir = os.path.join(output_base_dir, outfile_name)
        write_args['output_dir'] = output_dir
        python_cmd = write_python_command(**write_args)
        job_name = "_".join([ga_str, lr_str, temp_str, update_str])
        # job_name += "_3steps"
