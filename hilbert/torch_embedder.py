...
# TODO: enable sharding
class TorchHilbertEmbedderOptimized(object):
    ...
    def reset(self):
        self.V = self.sample_sphere()
        self.temp_V = torch.zeros(self.V.shape)
        self.temp_M = torch.zeros(self.M.shape)
        self.temp_delta = torch.zeros(self.M.shape)
        if self.one_sided:
            self.W = self.V.t()
        else:
            self.W = self.sample_sphere().t()
            self.temp_W = torch.zeros(self.W.shape)
        self.badness = None
    ...
    def get_gradient(self, offsets=None, pass_args=None):
        ...
        M_hat = torch.mm(use_W, use_V, out=self.temp_M)
        ...
        # Determine the gradient
        nabla_V = torch.mm(use_W.t(), delta, out=self.temp_V)

        if self.one_sided:
            return nabla_V

        nabla_W = torch.mm(delta, use_V.t(), out=self.temp_W)
        return nabla_V, nabla_W
    ...
    def update_self(self, pass_args=None):
        if self.one_sided:
            nabla_V = self.get_gradient(pass_args=pass_args)
            self.V += nabla_V.mul_(self.learning_rate)
        else:
            nabla_V, nabla_W = self.get_gradient(pass_args=pass_args)
            self.V += nabla_V.mul_(self.learning_rate)
            self.W += nabla_W.mul_(self.learning_rate)
