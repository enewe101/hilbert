...
class TwoBigramLoaderBase():

    def __init__(
        self, bigram_path, bigram_path2, sector_factor, shard_factor, num_loaders, 
        t_clean_undersample=None, alpha_unigram_smoothing=None,
        queue_size=1, device=None, verbose=True
    ):

        """
        Same as BigramLoaderBase(), but loads two bigrams
        """
        if num_loaders != sector_factor**2:
            warnings.warn(
                "`num_loaders` must equal `sector_factor**2`, so that each "
        ...
        self.bigram_path = bigram_path
        self.bigram_path2 = bigram_path2
        self.sector_factor = sector_factor
        self.shard_factor = shard_factor
        self.t_clean_undersample = t_clean_undersample
        self.alpha_unigram_smoothing = alpha_unigram_smoothing
    ...
    def _preload_iter(self, loader_id):
        ...
        for i, sector_id in enumerate(h.shards.Shards(self.sector_factor)):
            ...
            if self.loaded_sector != sector_id:
                ...
                self.bigram_sector2 = h.bigram.BigramSector.load(
                    self.bigram_path2, sector_id)
                self.bigram_sector2.apply_w2v_undersampling(
                    self.t_clean_undersample)
                self.bigram_sector2.apply_unigram_smoothing(
                    self.alpha_unigram_smoothing)
            ...
            for shard_id in h.shards.Shards(self.shard_factor):
                ...
                bigram_data2 = self.bigram_sector2.load_relative_shard(
                    shard=shard_id, device='cpu')
                unigram_data2 = self.bigram_sector2.load_relative_unigram_shard(
                    shard=shard_id, device='cpu')
                yield shard_id * sector_id, bigram_data, unigram_data, bigram_data2, unigram_data2


    def _load(self, preloaded, preloaded2):
        device = self.device or h.CONSTANTS.MATRIX_DEVICE
        shard_id, bigram_data, unigram_data = preloaded
        shard_id2, bigram_data2, unigram_data2 = preloaded2
        bigram_data = tuple(tensor.to(device) for tensor in bigram_data)
        unigram_data = tuple(tensor.to(device) for tensor in unigram_data)
        bigram_data2 = tuple(tensor.to(device) for tensor in bigram_data2)
        unigram_data2 = tuple(tensor.to(device) for tensor in unigram_data2)
        return shard_id, bigram_data, unigram_data


    def describe(self):
        s = '\tbigram_path = {}\n'.format(self.bigram_path)
        s += '\tbigram_path2 = {}\n'.format(self.bigram_path2)
        s += '\tsector_factor = {}\n'.format(self.sector_factor)
        s += '\tshard_factor = {}\n'.format(self.shard_factor)
        s += '\tnum_loaders = {}\n'.format(self.num_loaders)
        s += '\tt_clean_undersample = {}\n'.format(self.t_clean_undersample)
...
class AddiffLoader(TwoBigramLoaderBase):
    def __init__(
        self,bigram_path, bigram_path2, sector_factor, shard_factor, num_loaders, 
        t_clean_undersample=None, alpha_unigram_smoothing=None,
        queue_size=1, device=None, verbose=True, diff=5
    ):
        super(AddiffLoaderd, self).__init__(
            bigram_path, bigram_path2, sector_factor, shard_factor, num_loaders, 
            t_clean_undersample=t_clean_undersample, alpha_unigram_smoothing=alpha_unigram_smoothing,
            queue_size=queue_size, device=device, verbose=verbose
        )
    ...
    def _load(self, preloaded):
        """
        Adding diffusion with window sizing: 
        """
        device = self.device or h.CONSTANTS.MATRIX_DEVICE
        shard_id, bigram_data, unigram_data, bigram_data2, unigram_data2 = preloaded
        Nxx, Nx, Nxt, N = tuple(tensor.to(device) for tensor in bigram_data)
        window_Nxx, window_Nx, window_Nxt, window_N = tuple(tensor.to(device) for tensor in bigram_data2)
        trans_M = Nxx / Nx
        altered = torch.zeros(trans_M.size(), dtype=h.CONSTANTS.DEFAULT_DTYPE, device=device)
        denom = 0
        for i in range(self.w):
        ...
        Pxx_diff = altered * pi
        diff_Nxx = Pxx_diff * N
        new_Nxx = diff_Nxx + window_Nxx
        new_N = torch.sum(new_Nxx)
        new_Nx = torch.sum(new_Nxx, dim=1).view([3,1])
        new_Nxt = torch.sum(new_Nxx, dim=0).view([1,3])

        Pxx_data = new_Nxx / new_N
        Pxx_independent = torch.mm(new_Nx/new_N, new_Nxt/new_N)
        return shard_id, {
            'Pxx_data' : Pxx_data, 'Pxx_independent' : Pxx_independent
        }

    def describe(self):
        return 'Addition diffusion sharder \n' + super(
            AddiffLoader, self).describe()
