...
class TestArcLabelSampleLoader(TestCase):
    ...
    def test_arc_label_sample_loader(self):
        ...
        # Calculate the sampled expectation and probability.
        Nxx_sample = torch.zeros((num_labels,vocab,vocab), dtype=torch.float32)
        Vxx_sample = torch.zeros((num_labels,vocab,vocab), dtype=torch.float32)
        ...
        for sample_num in range(num_samples):
            ...
            for m in range(batch_size):
                ...
                Nxx_sample[k,i,j] += 1
                Vxx_sample[k,i,j] += exp_pmis['exp_pmi'][m]
        
        Qxx_sample = Nxx_sample / num_draws
        Pxx_sample = Vxx_sample / num_draws

        # Now calculate samples cooccurrence.
        cooc = h.cooccurrence.Cooccurrence.load(cooc_path)
        Nxx = torch.zeros((num_labels,vocab,vocab), dtype=torch.float32)

        for k in range(num_labels):
            Nxx_str = "Nxx_" + str(k) + ".npz"
            Nxx[k] = torch.tensor(sparse.load_npz(
                os.path.join(cooc_path, Nxx_str)).toarray(), dtype=torch.float32)
